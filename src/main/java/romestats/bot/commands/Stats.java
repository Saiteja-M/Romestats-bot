...
public class Stats extends Command {
	...
	public Stats() {
		this.name = "stats";
		this.help = "shows rome stats of the specfied soldier";
	...
	protected void execute(CommandEvent event) {
		...
		Unirest.get(url).asJson().ifSuccess((res)->{
			...
			int bonusScore =  (data.has("score")) ? data.getInt("sc_bonus") : 0;

			String rankImage = RANKS_URL + rank + ".png";

			EmbedBuilder embed = new EmbedBuilder();

			embed.setTitle("Romestats - " + args, UNITS_ROMESTATS + args).setThumbnail(rankImage)
