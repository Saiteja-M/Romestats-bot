...
public class WeaponStats extends Command {
	...
	public WeaponStats(EventWaiter waiter) {
		this.name = "wstats";
		this.help = "shows weapon stats of the specfied soldier";
	...
	protected void execute(CommandEvent event) {		
		...
		Unirest.get(url).asJson().ifSuccess((res)->{
			fetchWeaponStats(res.getBody().getObject(), event);
		}).ifFailure((res) -> {
			try {
	...
	private void fetchWeaponStats(JSONObject jsonResponse, CommandEvent event) {
		...
		List<WeaponObj> weaponsData = new ArrayList<WeaponObj>();
			
		ConfigLoader.KEY_CONFIG.getJSONArray("Weapons").forEach((config) -> {
	...
	private void buildTable(List<WeaponObj> weaponsData) {
		TableStringBuilder<WeaponObj> t = new TableStringBuilder<WeaponObj>();
